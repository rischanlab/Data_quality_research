...
if __name__ == "__main__":
    ...
    for k in k_list:
        ...
        for i in range(10):
            file_name = 'raw_results/db_missing_zipf_measure' + str(i+1) + '.xlsx'
    ...
    for k in k_list:
        ...
        for i in range(10):
            ...
            file_name = 'raw_results/db_missing_zipf_op_measure' + str(i + 1) + '.xlsx'
            for file_view in glob.glob(file_name):
                #print(percent, file_name, k)
                missing = ag.get_topk_aggregate(k, file_view)
                print("Missing topk: ", missing)
                ...
                with open('results/missing_zipf_op_measures_vs_ideal.csv', 'a', newline='') as f:
                    fields = [k, ag.rboresult(missing, topk), ag.jaccard_similarity(missing, topk)]
                    writer = csv.writer(f)
                    writer.writerow(fields)