...
if __name__ == "__main__":
    ...
    for my in my_list:
        ...
        for percent in percentage_list:
            for i in range(10):
                file_name0 = '5k_m_' + str(my) + 'k/db_' +str(percent) + 'diab' + str(i+1) + '.xlsx'
                #file_name1 = '10_percent_missing_m_3k/standard_db_' + str(percent) + 'diab' + str(i + 1) + '.xlsx'
                for impute_file in glob.glob(file_name0):
                    #print(len(impute_file))

                    if len(impute_file) == 23:
                        standard_file = '5k_m_' + str(my) + 'k/standard_' + impute_file[-15:]
                        #print(standard_file)
                    else:
                        standard_file = '5k_m_' + str(my) + 'k/standard_' + impute_file[-16:]
                        #print(standard_file)
                    ...
                    with open('results/dynamic_vs_standard.csv', 'a', newline='') as f:
                        fields = [percent, my, ag.rboresult(dynamic, standard), ag.jaccard_similarity(dynamic, standard)]
                        writer = csv.writer(f)
