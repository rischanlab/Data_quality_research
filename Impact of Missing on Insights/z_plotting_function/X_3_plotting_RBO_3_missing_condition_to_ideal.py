...
output_plot = 'RBO_three_missing_condition_vs_ideal'
...
ax.plot(mean3, lw = 1, color = '#ece554', alpha = 1, label = 'RBO 0.95 Missing Measure (M) to Ideal', marker='<', linestyle='-', linewidth=2, markersize=12)
...
ax.plot(mean8, lw = 1, color = '#5be19a', alpha = 1, label = 'RBO 0.95 Missing A and M to Ideal', marker='x', linestyle='-', linewidth=2, markersize=12)
...
ax.fill_between(t, lb8, ub8, color = '#5be19a', alpha = 0.4)


# Label the axes and provide a title
ax.set_title("Impact of k on Effectiveness - 95% CI, 10 % missing")
ax.set_xlabel("k")
ax.set_ylabel("Effectiveness - RBO 0.95 missing with 3 conditions to Ideal")
x = [5, 10, 15, 20]
xi = list(range(len(x)))
plt.xticks(xi, x)
# Display legend
