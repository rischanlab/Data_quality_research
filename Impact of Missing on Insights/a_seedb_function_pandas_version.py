...
class SeeDB:
    ...
    def query(self):
        df = psql.read_sql("SELECT * FROM " + self.table, self.conn)
        df.drop(df.columns[[0]], axis=1, inplace=True)

        df1 = df[df['readmitted'] == 'NO'].groupby([self.attribute2])[[self.attribute1]].agg([self.func]).reset_index()
        df1 = df1.sort_values([self.attribute2], ascending=[1])
        data1 = [tuple(x) for x in df1.values]

        df2 = df.groupby([self.attribute2])[[self.attribute1]].agg([self.func]).reset_index()
        df2 = df2.sort_values([self.attribute2], ascending=[1])
        data2 = [tuple(x) for x in df2.values]
        return data1, data2
    ...
    def store_result(self, table):
        ...
        writer = pd.ExcelWriter('test_results/' + table + '.xlsx')
