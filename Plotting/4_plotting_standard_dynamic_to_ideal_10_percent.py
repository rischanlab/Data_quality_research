...
input_file_dynamic_vs_ideal = 'results/dynamic_vs_ideal_10_percent.csv'
input_file_standard_vs_ideal = 'results/standard_vs_ideal_10_percent.csv'


output_plot_standard_dynamic_vs_ideal = 'standard_dynamic_vs_ideal_10_percent'


# ===============================================================
# STANDARD DYNAMIC VS IDEAL

df = pd.read_csv(input_file_dynamic_vs_ideal, names=['percentage','k','RBO','Jaccard'])
...
df1 = pd.read_csv(input_file_standard_vs_ideal, names=['percentage','k','RBO','Jaccard'])
...
ax.plot(mean0, lw = 1, color = '#539caf', alpha = 1, label = 'Jaccard Selective-imputation to Ideal', marker='o', linestyle='dashed', linewidth=2, markersize=12)
ax.plot(mean1, lw = 1, color = '#b65332', alpha = 1, label = 'RBO 0.95 Selective-imputation to Ideal', marker='s', linestyle=':', linewidth=2, markersize=12)

ax.plot(mean2, lw = 1, color = '#5be19a', alpha = 1, label = 'Jaccard Impute-all to Ideal', marker='x', linestyle='-.', linewidth=2, markersize=12)
ax.plot(mean3, lw = 1, color = '#ece554', alpha = 1, label = 'RBO 0.95 Impute-all to Ideal', marker='<', linestyle='-', linewidth=2, markersize=12)
...
ax.set_title("Impact of k on Effectiveness - 95% CI, m = 3k, 10 % missing")
ax.set_xlabel("k")
ax.set_ylabel("Effectiveness - Impute-all and Selective-imputation to Ideal")
x = [5, 10, 15, 20]
xi = list(range(len(x)))
plt.xticks(xi, x)
# Display legend
...
plt.savefig('plot/' + output_plot_standard_dynamic_vs_ideal, format="svg", dpi = 1000)
plt.savefig('plot/' + output_plot_standard_dynamic_vs_ideal + '.png')
plt.show()
