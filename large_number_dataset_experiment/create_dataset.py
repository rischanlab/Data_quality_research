...
for i in mlist:
    for j in range(10):
        #print(int(i * 100), j)
        x = int(i * 100)
        table_name = "db_" + str(x) + "mdiab" + str(j+1)
        #print(table_name)
        data = util.missing_data(db, i)
        engine = create_engine(r'postgresql+psycopg2://postgres:zenvisage@localhost:5432/diab_experiment')
        c = engine.connect()
        conn = c.connection
...
for i in mlist:
    for j in range(10):
        #print(int(i * 100), j)
        x = int(i * 100)
        table_name = "db_" + str(x) + "diab" + str(j+1)
        #print(table_name)
        data = util.missing_impute_data(db, i)
        engine = create_engine(r'postgresql+psycopg2://postgres:zenvisage@localhost:5432/diab_experiment')
        c = engine.connect()
        conn = c.connection
        print("Exporting ... ", table_name)
